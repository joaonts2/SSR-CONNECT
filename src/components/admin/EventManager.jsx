...
export default function EventManager() {
  ...
  const save = async (e) => {
    e.preventDefault();
    setSaving(true);
    if (editing === "new") await base44.entities.CalendarEvent.create(form);
    else await base44.entities.CalendarEvent.update(editing, form);
    setSaving(false); setEditing(null); load();
  };
  const remove = async (id) => {
    if (confirm("Excluir este evento?")) { await base44.entities.CalendarEvent.delete(id); load(); }
  };
  const set = (k) => (e) =>
